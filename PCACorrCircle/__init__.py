...
def create_correlation_table(A, B, names_cols_A, names_cols_B):
    ...
    A and B: 2d matrixes. The columns represent the different variables and the rows are the samples of thos variables
...
def pca_correlation_graph(X, variables_names, dimensions=(1, 2), figure_axis_size=6,X_pca=None):
    ...
    X = 2d matrix (np array, pandas, lists...). The columns represent the different variables and the rows are the samples of thos variables
    names_cols_X : name to be added to the final pandas table.
